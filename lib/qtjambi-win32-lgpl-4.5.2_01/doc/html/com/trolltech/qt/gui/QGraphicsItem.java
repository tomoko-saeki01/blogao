    ...
    implements com.trolltech.qt.gui.QGraphicsItemInterface
{
    ...
    private static class ConcreteWrapper extends com.trolltech.qt.gui.QGraphicsItem {
        ...
        public void paint(com.trolltech.qt.gui.QPainter painter, com.trolltech.qt.gui.QStyleOptionGraphicsItem option, com.trolltech.qt.gui.QWidget widget) {
            if (nativeId() == 0)
                throw new QNoNativeResourcesException("Function call on incomplete object of type: " +getClass().getName());
            super.__qt_paint_QPainter_nativepointer_QWidget(nativeId(), painter == null ? 0 : painter.nativeId(), option, widget == null ? 0 : widget.nativeId());
        }
    }
/**
This enum describes {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem}'s cache modes. Caching is used to speed up rendering by allocating and rendering to an off-screen pixel buffer, which can be reused when the item requires redrawing. For some paint devices, the cache is stored directly in graphics memory, which makes rendering very quick. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItem#setCacheMode(com.trolltech.qt.gui.QGraphicsItem.CacheMode) QGraphicsItem::setCacheMode()}. <br></DD></DT>
*/


    public enum CacheMode implements com.trolltech.qt.QtEnumerator {
/**
 The default; all item caching is disabled. {@link com.trolltech.qt.gui.QGraphicsItem#paint(com.trolltech.qt.gui.QPainter, com.trolltech.qt.gui.QStyleOptionGraphicsItem) QGraphicsItem::paint()} is called every time the item needs redrawing.
*/

        NoCache(0),
/**
 Caching is enabled for the item's logical (local) coordinate system. {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} creates an off-screen pixel buffer with a configurable size / resolution that you can pass to {@link com.trolltech.qt.gui.QGraphicsItem#setCacheMode(com.trolltech.qt.gui.QGraphicsItem.CacheMode) QGraphicsItem::setCacheMode()}. Rendering quality will typically degrade, depending on the resolution of the cache and the item transformation. The first time the item is redrawn, it will render itself into the cache, and the cache is then reused for every subsequent expose. The cache is also reused as the item is transformed. To adjust the resolution of the cache, you can call {@link com.trolltech.qt.gui.QGraphicsItem#setCacheMode(com.trolltech.qt.gui.QGraphicsItem.CacheMode) setCacheMode()} again.
*/

        ItemCoordinateCache(1),
/**
 Caching is enabled at the paint device level, in device coordinates. This mode is for items that can move, but are not rotated, scaled or sheared. If the item is transformed directly or indirectly, the cache will be regenerated automatically. Unlike ItemCoordinateCacheMode, {@link com.trolltech.qt.gui.QGraphicsItem.CacheMode DeviceCoordinateCache } always renders at maximum quality.
*/
...
/**
<brief>Returns the QGraphicsItem$CacheMode constant with the specified <tt>int</tt>.</brief>
*/
...
/**
<brief>Returns the QGraphicsItem$enum_1 constant with the specified <tt>int</tt>.</brief>
*/
...
/**
This enum describes different flags that you can set on an item to toggle different features in the item's behavior. <p>All flags are disabled by default.
*/

    public enum GraphicsItemFlag implements com.trolltech.qt.QtEnumerator {
/**
 The item supports interactive movement using the mouse. By clicking on the item and then dragging, the item will move together with the mouse cursor. If the item has children, all children are also moved. If the item is part of a selection, all selected items are also moved. This feature is provided as a convenience through the base implementation of {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem}'s mouse event handlers.
*/

        ItemIsMovable(1),
/**
 The item supports selection. Enabling this feature will enable {@link com.trolltech.qt.gui.QGraphicsItem#setSelected(boolean) setSelected()} to toggle selection for the item. It will also let the item be selected automatically as a result of calling {@link com.trolltech.qt.gui.QGraphicsScene#setSelectionArea(com.trolltech.qt.gui.QPainterPath) QGraphicsScene::setSelectionArea()}, by clicking on an item, or by using rubber band selection in {@link com.trolltech.qt.gui.QGraphicsView QGraphicsView}.
*/

        ItemIsSelectable(2),
/**
 The item supports keyboard input focus (i. ., it is an input item). Enabling this flag will allow the item to accept focus, which again allows the delivery of key events to {@link com.trolltech.qt.gui.QGraphicsItem#keyPressEvent(com.trolltech.qt.gui.QKeyEvent) QGraphicsItem::keyPressEvent()} and {@link com.trolltech.qt.gui.QGraphicsItem#keyReleaseEvent(com.trolltech.qt.gui.QKeyEvent) QGraphicsItem::keyReleaseEvent()}.
*/

        ItemIsFocusable(4),
/**
 The item clips to its own shape. The item cannot draw or receive mouse, tablet, drag and drop or hover events outside ts shape. It is disabled by default. This behavior is enforced by {@link com.trolltech.qt.gui.QGraphicsView#drawItems(com.trolltech.qt.gui.QPainter, com.trolltech.qt.gui.QGraphicsItemInterface[], com.trolltech.qt.gui.QStyleOptionGraphicsItem[]) QGraphicsView::drawItems()} or {@link com.trolltech.qt.gui.QGraphicsScene#drawItems(com.trolltech.qt.gui.QPainter, com.trolltech.qt.gui.QGraphicsItemInterface[], com.trolltech.qt.gui.QStyleOptionGraphicsItem[]) QGraphicsScene::drawItems()}. This flag was introduced in Qt 4.3.
*/

        ItemClipsToShape(8),
/**
 The item clips the painting of all its descendants to its own shape. Items that are either direct or indirect children of this item cannot draw outside this item's shape. By default, this flag is disabled; children can draw anywhere. This behavior is enforced by {@link com.trolltech.qt.gui.QGraphicsView#drawItems(com.trolltech.qt.gui.QPainter, com.trolltech.qt.gui.QGraphicsItemInterface[], com.trolltech.qt.gui.QStyleOptionGraphicsItem[]) QGraphicsView::drawItems()} or {@link com.trolltech.qt.gui.QGraphicsScene#drawItems(com.trolltech.qt.gui.QPainter, com.trolltech.qt.gui.QGraphicsItemInterface[], com.trolltech.qt.gui.QStyleOptionGraphicsItem[]) QGraphicsScene::drawItems()}. This flag was introduced in Qt 4.3.
*/

        ItemClipsChildrenToShape(16),
/**
 The item ignores inherited transformations (i. ., its position is still anchored to its parent, but the parent or view rotation, zoom or shear transformations are ignored). This flag is useful for keeping text label items horizontal and unscaled, so they will still be readable if the view is transformed. When set, the item's view geometry and scene geometry will be maintained separately. You must call {@link com.trolltech.qt.gui.QGraphicsItem#deviceTransform(com.trolltech.qt.gui.QTransform) deviceTransform()} to map coordinates and detect collisions in the view. By default, this flag is disabled. This flag was introduced in Qt 4.3. <b>Note:</b> With this flag set you can still scale the item itself, and that scale transformation will influence the item's children.
*/
...
*/
        ...
        GraphicsItemFlag(int value) { this.value = value; }
/**
This function should return an integer value for the enum values of the enumeration that implements this interface.
*/
...
/**
<brief>This function creates a com.trolltech.qt.gui.QGraphicsItem$GraphicsItemFlags with the specified <tt>com.trolltech.qt.gui.QGraphicsItem$GraphicsItemFlag[]</tt> QGraphicsItem$GraphicsItemFlag values set.</brief>
*/

        public static GraphicsItemFlags createQFlags(GraphicsItemFlag ... values) {
            return new GraphicsItemFlags(values);
        }
/**
<brief>Returns the QGraphicsItem$GraphicsItemFlag constant with the specified <tt>int</tt>.</brief>
*/

        public static GraphicsItemFlag resolve(int value) {
            return (GraphicsItemFlag) resolve_internal(value);
        }
        private static Object resolve_internal(int value) {
            switch (value) {
    ...
    public static class GraphicsItemFlags extends com.trolltech.qt.QFlags<GraphicsItemFlag> {
        private static final long serialVersionUID = 1L;
/**
<brief>Creates a QGraphicsItem-GraphicsItemFlags with the specified <tt>com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlag[]</tt>. flags set.</brief>
*/

        public GraphicsItemFlags(GraphicsItemFlag ... args) { super(args); }
        public GraphicsItemFlags(int value) { setValue(value); }
    }
/**
{@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemVisibleHasChanged }, {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemEnabledHasChanged }, {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemSelectedHasChanged }, {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemParentHasChanged }, {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemSceneHasChanged } <p>This enum describes the state changes that are notified by {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) QGraphicsItem::itemChange()}. The notifications are sent as the state changes, and in some cases, adjustments can be made (see the documentation for each change for details). <p>Note: Be careful with calling functions on the {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} itself inside {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}, as certain function calls can lead to unwanted recursion. For example, you cannot call {@link com.trolltech.qt.gui.QGraphicsItem#setPos(com.trolltech.qt.core.QPointF) setPos()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} on an {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemPositionChange } notification, as the {@link com.trolltech.qt.gui.QGraphicsItem#setPos(com.trolltech.qt.core.QPointF) setPos()} function will again call itemChange({@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemPositionChange }). Instead, you can return the new, adjusted position from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/


    public enum GraphicsItemChange implements com.trolltech.qt.QtEnumerator {
/**
 The item's position changes. This notification is only sent when the item's local position changes, relative to its parent, has changed (i.e., as a result of calling {@link com.trolltech.qt.gui.QGraphicsItem#setPos(com.trolltech.qt.core.QPointF) setPos()} or {@link com.trolltech.qt.gui.QGraphicsItem#moveBy(double, double) moveBy()}). The value argument is the new position (i.e., a {@link com.trolltech.qt.core.QPointF QPointF}). You can call {@link com.trolltech.qt.gui.QGraphicsItem#pos() pos()} to get the original position. Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setPos(com.trolltech.qt.core.QPointF) setPos()} or {@link com.trolltech.qt.gui.QGraphicsItem#moveBy(double, double) moveBy()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered; instead, you can return the new, adjusted position from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}. After this notification, {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} immediately sends the {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemPositionHasChanged } notification if the position changed.
*/

        ItemPositionChange(0),
/**
 The item's affine transformation matrix is changing. This value is obsolete; you can use {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange ItemTransformChange } instead.
*/

        ItemMatrixChange(1),
/**
 The item's visible state changes. If the item is presently visible, it will become invisible, and vice verca. The value argument is the new visible state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setVisible(boolean) setVisible()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered; instead, you can return the new visible state from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemVisibleChange(2),
/**
 The item's enabled state changes. If the item is presently enabled, it will become disabled, and vice verca. The value argument is the new enabled state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setEnabled(boolean) setEnabled()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return the new state from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemEnabledChange(3),
/**
 The item's selected state changes. If the item is presently selected, it will become unselected, and vice verca. The value argument is the new selected state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setSelected(boolean) setSelected()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered(); instead, you can return the new selected state from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemSelectedChange(4),
/**
 The item's parent changes. The value argument is the new parent item (i.e., a {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} pointer). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setParentItem(com.trolltech.qt.gui.QGraphicsItemInterface) setParentItem()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered; instead, you can return the new parent from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemParentChange(5),
/**
 A child is added to this item. The value argument is the new child item (i.e., a {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} pointer). Do not pass this item to any item's {@link com.trolltech.qt.gui.QGraphicsItem#setParentItem(com.trolltech.qt.gui.QGraphicsItemInterface) setParentItem()} function as this notification is delivered. The return value is unused; you cannot adjust anything in this notification. Note that the new child might not be fully constructed when this notification is sent; calling pure virtual functions on the child can lead to a crash.
*/

        ItemChildAddedChange(6),
/**
 A child is removed from this item. The value argument is the child item that is about to be removed (i.e., a {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} pointer). The return value is unused; you cannot adjust anything in this notification.
*/

        ItemChildRemovedChange(7),
/**
 The item's transformation matrix changes. This notification is only sent when the item's local transformation matrix changes (i.e., as a result of calling {@link com.trolltech.qt.gui.QGraphicsItem#setTransform(com.trolltech.qt.gui.QTransform) setTransform()}, or one of the convenience transformation functions, such as {@link com.trolltech.qt.gui.QGraphicsItem#rotate(double) rotate()}). The value argument is the new matrix (i.e., a {@link com.trolltech.qt.gui.QTransform QTransform}); to get the old matrix, call {@link com.trolltech.qt.gui.QGraphicsItem#transform() transform()}. Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setTransform(com.trolltech.qt.gui.QTransform) setTransform()} or any of the transformation convenience functions in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered; instead, you can return the new matrix from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemTransformChange(8),
/**
 The item's position has changed. This notification is only sent after the item's local position, relative to its parent, has changed. The value argument is the new position (the same as {@link com.trolltech.qt.gui.QGraphicsItem#pos() pos()}), and {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} ignores the return value for this notification (i.e., a read-only notification).
*/

        ItemPositionHasChanged(9),
/**
 The item's transformation matrix has changed. This notification is only sent after the item's local trasformation matrix has changed. The value argument is the new matrix (same as {@link com.trolltech.qt.gui.QGraphicsItem#transform() transform()}), and {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem} ignores the return value for this notification (i.e., a read-only notification).
*/

        ItemTransformHasChanged(10),
/**
 The item is moved to a new scene. This notification is also sent when the item is added to its initial scene, and when it is removed. The value argument is the new scene (i.e., a {@link com.trolltech.qt.gui.QGraphicsScene QGraphicsScene} pointer), or a null pointer if the item is removed from a scene. Do not override this change by passing this item to {@link com.trolltech.qt.gui.QGraphicsScene#addItem(com.trolltech.qt.gui.QGraphicsItemInterface) QGraphicsScene::addItem()} as this notification is delivered; instead, you can return the new scene from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}. Use this feature with caution; objecting to a scene change can quickly lead to unwanted recursion.
*/

        ItemSceneChange(11),
/**
 The item's visible state has changed. The value argument is the new visible state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setVisible(boolean) setVisible()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemVisibleHasChanged(12),
/**
 The item's enabled state has changed. The value argument is the new enabled state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setEnabled(boolean) setEnabled()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemEnabledHasChanged(13),
/**
 The item's selected state has changed. The value argument is the new selected state (i.e., true or false). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setSelected(boolean) setSelected()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemSelectedHasChanged(14),
/**
 The item's parent has changed. The value argument is the new parent (i.e., a pointer to a {@link com.trolltech.qt.gui.QGraphicsItem QGraphicsItem}). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setParentItem(com.trolltech.qt.gui.QGraphicsItemInterface) setParentItem()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemParentHasChanged(15),
/**
 The item's scene has changed. The value argument is the new scene (i.e., a pointer to a {@link com.trolltech.qt.gui.QGraphicsScene QGraphicsScene}). Do not call setScene() in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemSceneHasChanged(16),
/**
 The item's cursor changes. The value argument is the new cursor (i.e., a {@link com.trolltech.qt.gui.QCursor QCursor}). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setCursor(com.trolltech.qt.gui.QCursor) setCursor()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return a new cursor from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemCursorChange(17),
/**
 The item's cursor has changed. The value argument is the new cursor (i.e., a {@link com.trolltech.qt.gui.QCursor QCursor}). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setCursor(com.trolltech.qt.gui.QCursor) setCursor()} as this notification is delivered. The return value is ignored.
*/

        ItemCursorHasChanged(18),
/**
 The item's tooltip changes. The value argument is the new tooltip (i.e., a {@link com.trolltech.qt.gui.QToolTip QToolTip}). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setToolTip(java.lang.String) setToolTip()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return a new tooltip from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemToolTipChange(19),
/**
 The item's tooltip has changed. The value argument is the new tooltip (i.e., a {@link com.trolltech.qt.gui.QToolTip QToolTip}). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setToolTip(java.lang.String) setToolTip()} as this notification is delivered. The return value is ignored.
*/

        ItemToolTipHasChanged(20),
/**
 The item's flags change. The value argument is the new flags (i.e., a quint32). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setFlags(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlag[]) setFlags()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return the new flags from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemFlagsChange(21),
/**
 The item's flags have changed. The value argument is the new flags (i.e., a quint32). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setFlags(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlag[]) setFlags()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. The return value is ignored.
*/

        ItemFlagsHaveChanged(22),
/**
 The item's Z-value changes. The value argument is the new Z-value (i.e., a double). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setZValue(double) setZValue()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return a new Z-value from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemZValueChange(23),
/**
 The item's Z-value has changed. The value argument is the new Z-value (i.e., a double). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setZValue(double) setZValue()} as this notification is delivered. The return value is ignored.
*/

        ItemZValueHasChanged(24),
/**
 The item's opacity changes. The value argument is the new opacity (i.e., a double). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setOpacity(double) setOpacity()} in {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()} as this notification is delivered. Instead, you can return a new opacity from {@link com.trolltech.qt.gui.QGraphicsItem#itemChange(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange, java.lang.Object) itemChange()}.
*/

        ItemOpacityChange(25),
/**
 The item's opacity has changed. The value argument is the new opacity (i.e., a double). Do not call {@link com.trolltech.qt.gui.QGraphicsItem#setOpacity(double) setOpacity()} as this notification is delivered. The return value is ignored.
*/

        ItemOpacityHasChanged(26);

        GraphicsItemChange(int value) { this.value = value; }
/**
This function should return an integer value for the enum values of the enumeration that implements this interface.
*/
...
/**
<brief>Returns the QGraphicsItem$GraphicsItemChange constant with the specified <tt>int</tt>.</brief>
*/

        public static GraphicsItemChange resolve(int value) {
            return (GraphicsItemChange) resolve_internal(value);
        }
        private static Object resolve_internal(int value) {
            switch (value) {
            case 0: return ItemPositionChange;
            case 1: return ItemMatrixChange;
...
/**
<brief>Returns the QGraphicsItem$Extension constant with the specified <tt>int</tt>.</brief>
*/
...
*/
    ...
    public QGraphicsItem(com.trolltech.qt.gui.QGraphicsItemInterface parent) {
...
*/
    ...
    public QGraphicsItem(com.trolltech.qt.gui.QGraphicsItemInterface parent, com.trolltech.qt.gui.QGraphicsScene scene){
        super((QPrivateConstructor)null);
        __qt_QGraphicsItem_QGraphicsItem_QGraphicsScene(parent == null ? 0 : parent.nativeId(), scene == null ? 0 : scene.nativeId());
        if (parent != null)
    ...
    native void __qt_QGraphicsItem_QGraphicsItem_QGraphicsScene(long parent, long scene);
...
*/
    ...
    @QtBlockedSlot
    public final java.util.List<com.trolltech.qt.gui.QGraphicsItemInterface> collidingItems() {
        return collidingItems(com.trolltech.qt.core.Qt.ItemSelectionMode.IntersectsItemShape);
    }
/**
...
*/
    ...
    public final void ensureVisible() {
        ensureVisible((com.trolltech.qt.core.QRectF)new com.trolltech.qt.core.QRectF(), (int)50, (int)50);
    }
/**
If this item is part of a scene that is viewed by a {@link com.trolltech.qt.gui.QGraphicsView QGraphicsView}, this convenience function will attempt to scroll the view to ensure that <tt>rect</tt> is visible inside the view's viewport. If <tt>rect</tt> is a null rect (the default), QGraphicsItem will default to the item's bounding rect. <tt>xmargin</tt> and <tt>ymargin</tt> are the number of pixels the view should use for margins. <p>If the specified rect cannot be reached, the contents are scrolled to the nearest valid position. <p>If this item is not viewed by a {@link com.trolltech.qt.gui.QGraphicsView QGraphicsView}, this function does nothing. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsView#ensureVisible(com.trolltech.qt.gui.QGraphicsItemInterface, int) QGraphicsView::ensureVisible()}. <br></DD></DT>
*/
...
*/
    ...
    public final void scroll(double dx, double dy) {
        scroll(dx, dy, (com.trolltech.qt.core.QRectF)new com.trolltech.qt.core.QRectF());
    }
/**
Scrolls the contents of <tt>rect</tt> by <tt>dx</tt>, <tt>dy</tt>. If <tt>rect</tt> is a null rect (the default), the item's bounding rect is scrolled. <p>Scrolling provides a fast alternative to simply redrawing when the contents of the item (or parts of the item) are shifted vertically or horizontally. Depending on the current transformation and the capabilities of the paint device (i.e., the viewport), this operation may consist of simply moving pixels from one location to another using memmove(). In most cases this is faster than rerendering the entire area. <p>After scrolling, the item will issue an update for the newly exposed areas. If scrolling is not supported (e.g., you are rendering to an OpenGL viewport, which does not benefit from scroll optimizations), this function is equivalent to calling update(<tt>rect</tt>). <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItem#boundingRect() boundingRect()}. <br></DD></DT>
*/
...
*/
    ...
    @QtBlockedSlot
    public final void setAcceptedMouseButtons(com.trolltech.qt.core.Qt.MouseButton ... buttons) {
        this.setAcceptedMouseButtons(new com.trolltech.qt.core.Qt.MouseButtons(buttons));
    }
/**
...
*/
    ...
    @QtBlockedSlot
    public final void setFlag(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlag flag) {
...
*/
    ...
    @QtBlockedSlot
    public final void setFlags(com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlag ... flags) {
        this.setFlags(new com.trolltech.qt.gui.QGraphicsItem.GraphicsItemFlags(flags));
    }
/**
...
/**
This virtual function is called twice for all items by the {@link com.trolltech.qt.gui.QGraphicsScene#advance() QGraphicsScene::advance()} slot. In the first phase, all items are called with <tt>phase</tt> == 0, indicating that items on the scene are about to advance, and then all items are called with <tt>phase</tt> == 1. Reimplement this function to update your item if you need simple scene-controlled animation. <p>The default implementation does nothing. <p>For individual item animation, an alternative to this function is to either use {@link com.trolltech.qt.gui.QGraphicsItemAnimation QGraphicsItemAnimation}, or to multiple-inherit from {@link com.trolltech.qt.core.QObject QObject} and QGraphicsItem, and animate your item using {@link com.trolltech.qt.core.QObject#startTimer(int) QObject::startTimer()} and {@link com.trolltech.qt.core.QObject#timerEvent(com.trolltech.qt.core.QTimerEvent) QObject::timerEvent()}. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItemAnimation QGraphicsItemAnimation}, and {@link com.trolltech.qt.core.QTimeLine QTimeLine}. <br></DD></DT>
*/
...
*/
    ...
    @QtBlockedSlot
    public abstract com.trolltech.qt.core.QRectF boundingRect();
    @QtBlockedSlot
...
*/
    ...
    @QtBlockedSlot
    public final boolean collidesWithItem(com.trolltech.qt.gui.QGraphicsItemInterface other) {
        return collidesWithItem(other, com.trolltech.qt.core.Qt.ItemSelectionMode.IntersectsItemShape);
    }
/**
Returns true if this item collides with <tt>other</tt>; otherwise returns false. The ways items collide is determined by <tt>mode</tt>. The default value for <tt>mode</tt> is {@link com.trolltech.qt.core.Qt.ItemSelectionMode Qt::IntersectsItemShape }; <tt>other</tt> collides with this item if it either intersects, contains, or is contained by this item's shape. <p>The default implementation is based on shape intersection, and it calls {@link com.trolltech.qt.gui.QGraphicsItem#shape() shape()} on both items. Because the complexity of arbitrary shape-shape intersection grows with an order of magnitude when the shapes are complex, this operation can be noticably time consuming. You have the option of reimplementing this function in a subclass of QGraphicsItem to provide a custom algorithm. This allows you to make use of natural constraints in the shapes of your own items, in order to improve the performance of the collision detection. For instance, two untransformed perfectly circular items' collision can be determined very efficiently by comparing their positions and radii. <p>Keep in mind that when reimplementing this function and calling {@link com.trolltech.qt.gui.QGraphicsItem#shape() shape()} or {@link com.trolltech.qt.gui.QGraphicsItem#boundingRect() boundingRect()} on <tt>other</tt>, the returned coordinates must be mapped to this item's coordinate system before any intersection can take place. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItem#contains(com.trolltech.qt.core.QPointF) contains()}, and {@link com.trolltech.qt.gui.QGraphicsItem#shape() shape()}. <br></DD></DT>
*/
...
*/
    ...
    @QtBlockedSlot
    public final boolean collidesWithPath(com.trolltech.qt.gui.QPainterPath path) {
        return collidesWithPath(path, com.trolltech.qt.core.Qt.ItemSelectionMode.IntersectsItemShape);
    }
/**
Returns true if this item collides with <tt>path</tt>. <p>The collision is determined by <tt>mode</tt>. The default value for <tt>mode</tt> is {@link com.trolltech.qt.core.Qt.ItemSelectionMode Qt::IntersectsItemShape }; <tt>path</tt> collides with this item if it either intersects, contains, or is contained by this item's shape. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItem#collidesWithItem(com.trolltech.qt.gui.QGraphicsItemInterface) collidesWithItem()}, {@link com.trolltech.qt.gui.QGraphicsItem#contains(com.trolltech.qt.core.QPointF) contains()}, and {@link com.trolltech.qt.gui.QGraphicsItem#shape() shape()}. <br></DD></DT>
*/
...
</pre> The default implementation does nothing, and returns <tt>value</tt>. <p>Note: Certain QGraphicsItem functions cannot be called in a reimplementation of this function; see the {@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange GraphicsItemChange } documentation for details. <p><DT><b>See also:</b><br><DD>{@link com.trolltech.qt.gui.QGraphicsItem.GraphicsItemChange GraphicsItemChange }. <br></DD></DT>
*/
...
*/
    ...
    @QtBlockedSlot
    public abstract void paint(com.trolltech.qt.gui.QPainter painter, com.trolltech.qt.gui.QStyleOptionGraphicsItem option, com.trolltech.qt.gui.QWidget widget);
    @QtBlockedSlot
...
/**
Returns the type of an item as an int. All standard graphicsitem classes are associated with a unique value; see QGraphicsItem::Type. This type information is used by qgraphicsitem_cast() to distinguish between types. <p>The default implementation (in QGraphicsItem) returns UserType. <p>To enable use of qgraphicsitem_cast() with a custom item, reimplement this function and declare a Type enum value equal to your custom item's type. Custom items must return a value larger than or equal to UserType (65536). <p>For example:Error parsing snippet.</pre> <p><DT><b>See also:</b><br><DD>UserType. <br></DD></DT>
*/
...
*/
    ...
    public static native QGraphicsItem fromNativePointer(QNativePointer nativePointer);
...
*/
    ...
    protected QGraphicsItem(QPrivateConstructor p) { super(p); } 
...
/**
<brief>Returns a string representation of the <tt>this QGraphicsItem</tt>. </brief>
*/
